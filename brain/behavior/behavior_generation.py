...
from brain.paths import PROPOSED_GOALS, FOCUS_GOAL
...
def generate_behavior_from_integration(context: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    try:
        # (T0.4) Seed VALID JSON ([]), not a 0-byte touch — ensure_files().touch()
        # left proposed_goals.json at 0 bytes, which is invalid JSON and breaks any
        # reader using raw json.loads (the guarded load_json here tolerates it, but
        # other readers don't).
        if (not PROPOSED_GOALS.exists()) or PROPOSED_GOALS.stat().st_size == 0:
            save_json(PROPOSED_GOALS, [])
