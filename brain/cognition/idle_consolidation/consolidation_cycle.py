...
def idle_consolidation_cycle(context: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    # Quality-standard evolution (T0.5) — non-cognition background pass: propose
    # promotions from downstream-credited work + flag suspect exemplars, auto-apply
    # only predicate-conforming promotions (loosening waits for the human queue).
    try:
        from brain.cognition.quality_standard import run_background_pass as _qse
        _qs = _qse(context)
        if any(_qs.values()):
            log_activity(f"[dream] Quality-standard pass: promotions="
                         f"{_qs['promotions_proposed']} suspects={_qs['suspects_flagged']} "
                         f"applied={_qs['applied']}")
    except Exception as _qsee:
        log_activity(f"[dream] quality-standard pass skipped: {_qsee}")
    ...
    # Threat/impasse discharge — the "release valve actually releases" fix (R3).
    _submit_retreat_discharge()
...
# Extracted to retreat_discharge.py (module-size budget); re-exported here so
# `consolidation_cycle._submit_retreat_discharge` stays the stable call site.
from brain.cognition.idle_consolidation.retreat_discharge import (
    submit_retreat_discharge as _submit_retreat_discharge,
)
