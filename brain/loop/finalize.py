...
def persist_and_periodic(context: Context, _goals_api: Any, _mem_daemon: Any, _evaluator: Any, affect_state: Any) -> Context:
    ...
    try:
        ...
        if (not _heavy_paused()) and (not _resource_floor_shedding()) and should_consolidate(context):
            ...
            _dt.start()
            # The dream IS the integration the rest drive demands. It runs on a
            # side thread, so its function name never reaches evaluate_cycle's
            # keyword discharge — satisfy the drive here, at the launch site.
            try:
                from brain.runtime_coupling import demand_engine as _demand_mod
                _demand_mod.satisfy("rest", 0.6)
            except Exception as _rde:
                record_failure("ORRIN_loop.rest_discharge", _rde)
