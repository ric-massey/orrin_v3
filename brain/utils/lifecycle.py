...
(§9.7) / Death Screen (§10.4) depend on distinguishing them:
  ...
  • interrupted  — the previous run did NOT shut down cleanly (a crash, or a supervisor
...
#   reaper → a watchdog stall/kill (Supervisor.trigger) → "stalled" (it restarts)
#           (the runstate key stays "reaper" — a frozen on-disk marker)
#   neither → an unclean exit with no supervisor trip    → "crashed"
...
def mark_running() -> None:
    ...
    (unclean, no supervisor trip) until a graceful shutdown or a supervisor trip updates it."""
...
def mark_stall(reason: str = "") -> None:
    """A watchdog (Supervisor.trigger) is taking Orrin down to restart him — NOT death,
...
def status() -> Dict[str, Any]:
    ...
    then a supervisor stall reads as 'stalled', any other unclean exit as 'crashed'."""
    state = "alive"
    info: Dict[str, Any] = {}
    try:
        ...
        if lifespan_rolled():
            ...
            # that final thoughts exist on disk. The supervisor's termination-window reflection (a
