# utils/ui_build.py
# Resilient Vite builder for UI dashboards
...
def ensure_ui_build(ui_name: str, dist_dir: Path) -> bool:
    """
    Ensure Vite 'dist/index.html' exists. If missing, try npm ci/install + npm run build.
    On macOS, attempt 'brew install node' if npm is missing.
    Returns True if dist is present after checks/build; else False.
    """
    ...
    if npm is None:
        print(f"[{ui_name}] npm not found. Please install Node.js, then run:\n"
              f"  cd {ui_src_dir}\n  npm install\n  npm run build")
        return False
