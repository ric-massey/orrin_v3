...
export const METRICS: MetricDef[] = [
  ...
  },
  {
    ...
    perspective: "agent-accessible",
    long: "Its available cognitive capacity right now — the opposite of fatigue. Sustained hard cognition (recursion, long goal pushes, heavy reflection) accumulates a 'resource deficit' (ego-depletion; Baumeister 1998); rest and easy cycles let it recover. Energy = 1 − that deficit.",
    terms: [
      { t: "Capacity", d: "Headroom for effortful, deliberate work. High capacity → it can plan, reason, persist. Low → it defaults to easy/automatic actions." },
  ...
  },
  {
    ...
    desc: "The accumulating resource deficit it's carrying.", lo: "rested", hi: "depleted",
    perspective: "agent-accessible",
    ...
    terms: [
      { t: "Resource deficit", d: "The accumulating drain. Ticks up ~+0.002 every cycle, faster under effortful function use; high values bias it toward easy/automatic actions over deliberate ones." },
      { t: "Allostatic setpoint (τ)", d: "The deficit target it relaxes toward — not fixed. ~0.12 idle, 0.15 neutral, 0.30 under user demand, 0.40 critical: it runs hotter when it matters, then forces recovery as load accrues (predictive regulation, not reactive)." },
  ...
  },
  {
    ...
    perspective: "agent-accessible",
    long: "Its appetitive drive — how strongly it wants to act, pursue, and finish. It's lifted by reward (completing things, value-aligned actions) and a flow bonus for sustained action, and decays toward a 0.5 baseline so it doesn't pin at the ceiling.",
    terms: [
  ...
  },
  {
    ...
    desc: "Self-trust in its current footing.", lo: "unsure", hi: "assured",
    perspective: "agent-accessible",
    long: "How much it trusts its current understanding/plan. It rises with successful, well-predicted actions and falls on surprise (prediction error) and contradiction. Confidence feeds regulation (high confidence → stay the course; low → reflect/verify).",
    terms: [
      { t: "Prediction error", d: "Gap between what it expected and what happened. Big errors cut confidence (and raise curiosity)." },
  ...
  },
  {
    ...
    perspective: "agent-accessible",
    long: "Its exploration drive: the pull toward novel/uncertain things. It's the same signal that, when high, makes it spawn investigation sub-goals on its own. Driven by novelty (how unlike anything known), uncertainty (poor coverage), and prediction error.",
    terms: [
      { t: "Novelty", d: "How unlike anything in memory a thing is (inverse structural-analogy score)." },
      { t: "Information gap (uncertainty)", d: "How poorly its rules/knowledge cover a topic — Loewenstein's curiosity sits in that gap." },
    ],
  ...
  },
  {
    ...
    perspective: "agent-accessible",
    long: "A single 'how much is it struggling' reading — the summed weight of its negative signals (impasse/frustration, threat, conflict, risk, rejection). High distress shortens its regulation cadence (it works harder to calm down) and biases it toward regulation functions.",
    terms: [
  ...
  },
  {
    ...
    desc: "How settled vs. volatile its signal state is.", lo: "volatile", hi: "steady",
    perspective: "agent-accessible",
  ...
  },
  {
    ...
    desc: "Share of its recent predictions that came true.", lo: "missing", hi: "on target",
    perspective: "dev-only",
    long: "Whether its model of the world is actually getting things right: the fraction of its recently-resolved predictions that came true. This is the live readout of the agency-based learning loop — it predicts effects of its actions, checks them, and the confirmed ones crystallize into rules.",
    terms: [
      { t: "Prediction", d: "A falsifiable expectation it commits to (e.g. 'after pursue_goal, impasse falls')." },
      { t: "Resolved", d: "A prediction that's been checked against what actually happened (correct / not)." },
    ],
    measure: "hits / resolved over its last ~40 predictions, refreshed every few seconds.",
