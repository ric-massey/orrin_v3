...
export const ROOM_INFO: Record<string, ValueInfo> = {
  // ── Cognition ──────────────────────────────────────────────────────────────
  workspace: {
    label: "Attention winner & competitors",
    what: "Each cycle, candidate contents compete on salience and exactly one wins the global workspace — that's what it's attending to. The list shows the field; the winner is the max-salience candidate.",
    src: { file: "brain/cognition/global_workspace.py", start: 136, end: 205, label: "update_workspace — salience competition → one winner" },
  },
  cog_goal: {
    label: "Active goal",
    what: "The goal it's currently pursuing, with its step/status — drawn from its live goal set.",
  ...
  symbolic: {
    ...
    what: "Rules it's learned that fire during reasoning, with how many times each has hit. 'rules_total' is the size of its learned rule set.",
    src: { file: "backend/server/app.py", start: 555, end: 598, label: "/api/symbolic — learned rules + hit counts" },
  },
  peers: {
    label: "Peer models",
    what: "The internal peer models that press on its deliberation — its social-presence models, even when alone.",
    src: { file: "backend/server/app.py", start: 698, end: 740, label: "/api/people — peer models" },
  },
  ...
  life_disk: {
    ...
    what: "Its data store's size on disk against the user's disk ceiling — 'room left to grow', not the raw host disk. The forgetting sweeps trim toward this ceiling.",
    src: { file: "brain/utils/resource_ceilings.py", start: 74, end: 79, label: "usage() — store size vs the disk ceiling" },
  },
  life_rate: {
    label: "Cycle rate",
  ...
  life_age: {
    label: "Uptime",
    what: "Uptime in days since start. Suspend-when-closed pauses the clock; this is real elapsed run time.",
    src: { file: "brain/cognition/mortality.py", start: 209, end: 240, label: "life_status — uptime + phase (estimate-only)" },
  },
  life_remaining: {
    label: "Est. runtime remaining",
    what: "Its internal estimate of remaining runtime — never the true horizon (that's deliberately never exposed). A trust/honesty boundary.",
  ...
  memory_list: {
    ...
    what: "Long-term memories ordered by recency or importance, with the matched/total counts. Importance is the per-memory weight its forgetting respects.",
    src: { file: "backend/server/app.py", start: 382, end: 440, label: "/api/memory — store query, order, counts" },
  },
  memory_forgotten: {
    label: "Forgotten",
    what: "Forgetting sweeps — what decayed or was pruned, and how many items. Watching it forget is what makes 'its memory stays bounded' real.",
