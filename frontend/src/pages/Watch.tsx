...
/**
 * Watch — the front door for someone who has never heard of Orrin.
 *
 * Goal: a newcomer glances and instantly understands "this is a mind, thinking
 * on its own, right now." No input box, no dashboard. Three reads, in order:
 *   1. it's ALIVE        → a breathing orb whose colour is its mood
 *   2. it's THINKING     → one big plain-language thought line
 *   3. ON ITS OWN        → a fading stream of the thoughts just before this one
 *
 * Everything obeys the bio↔eng toggle: the visual is identical in both
 * dialects, only the words change. That toggle is the whole "for everyone"
 * idea — the biological words welcome a beginner, the engineering words satisfy
 * a skeptic, and flipping it live over the SAME moving picture is the most
 * convincing proof there's real machinery underneath.
 */
export default function Watch() {
  ...
  const mood = moodWord(valence, arousal, mode);
  // Mood → colour: warm gold when he feels good, cool slate when he doesn't.
  // Arousal sets the breathing speed; calm breathes slow, activated breathes fast.
  ...
  // The "stream of consciousness": the deliberate thoughts just before this one,
  // newest first, fading with age — visible proof he's moving by himself.
  const trail = useMemo(() => {
  ...
  return (
    <div className="relative flex h-[calc(100dvh-3.5rem)] flex-col items-center justify-center overflow-hidden bg-[#070a10] text-foreground sm:h-[calc(100dvh-4rem)]">
      {/* ambient mood wash — colour follows his feeling, drifts slowly */}
      <div
      ...
      <div className="z-10 mt-10 flex min-h-[3.5rem] max-w-2xl flex-wrap items-center justify-center gap-x-1 gap-y-1 px-6 text-center">
        {trail.map((th, i) => (
          <Fragment key={th.key}>
            ...
            >
              {thoughtForFn(th.fn, mode)}
            </span>
          </Fragment>
        ))}
      </div>

      {/* bottom: the one explanation + the language switch (the demo's punchline) */}
      <div className="absolute bottom-0 left-0 right-0 z-20 flex flex-col items-center gap-3 px-6 pb-6">
        <p className="max-w-md text-center text-xs leading-relaxed text-white/40">
          You're watching Orrin think on his own. Switch the language below — the
          picture stays the same, only the words change.
        </p>
        <div
          role="tablist"
          aria-label="Language"
          className="flex items-center rounded-full border border-white/15 bg-white/5 p-0.5 text-[11px] font-medium backdrop-blur"
        >
          {([
            ["bio", "Like a brain"],
            ["eng", "Like a machine"],
            ...
            <button
              ...
              className={cn(
                ...
                mode === m ? "bg-white/90 text-[#0a0d14]" : "text-white/60 hover:text-white",
              )}
            >
              {label}
            </button>
...
// Same affect, two dialects (mirrors NarrativeStatusCard).
