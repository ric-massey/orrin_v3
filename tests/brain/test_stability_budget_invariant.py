...
# commit_signals actually applies must never exceed STABILITY_BUDGET, no matter
...
# formula commit_signals uses internally.
import random
...
def test_stability_budget_never_exceeded_random_proposals():
    ...
    for _trial in range(50):
        ...
        for _ in range(random.randint(1, 30)):
            ...
            submit_signal(ctx, target, delta, weight=weight, source="random")
...
def test_stability_budget_never_exceeded_single_signal_flood():
    ...
    for _trial in range(20):
        ...
        for _ in range(random.randint(5, 40)):
            submit_signal(ctx, target, sign * random.uniform(0.05, 0.2),
